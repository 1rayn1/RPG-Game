...
class Skill:
    def __init__(self, name, description, skill_type, mana_cost=0, cooldown=0,
...
def battle(stdscr, monster_name, stats, inventory, level_range=(1, 1)):
    monster = create_monster(monster_name, level_range)
    monster_hp, monster_attack, monster_speed = monster.hp, monster.attack, monster.speed
    player_hp = stats["hp"]
    player_attack = stats["attack"]
    player_defense = stats["defense"]
    player_speed = stats["speed"]

    # Get wielded weapon bonus if any
    weapon_damage = 0
    weapon_name = ""
    if stats.get("wielded_index") is not None:
        weapon = inventory[stats["wielded_index"]]
        weapon_damage = weapon.damage
        weapon_name = weapon.name

    stdscr.clear()
    stdscr.addstr(0, 0, f"A wild {monster.name} (Lv {monster.level}) appeared!")
    stdscr.addstr(1, 0, f"{monster.name} Stats - HP: {monster_hp}, ATK: {monster_attack}, SPD: {monster_speed}")
    stdscr.addstr(2, 0, f"You wield: {weapon_name} (+{weapon_damage} dmg)" if weapon_name else "You are unarmed.")
    stdscr.refresh()
    time.sleep(1)
    ...
    while player_hp > 0 and monster_hp > 0:
        ...
        if player_speed >= monster_speed:
            damage = max(player_attack + weapon_damage - 1, 1)
            monster_hp -= damage
            stdscr.addstr(0, 0, f"You attack {monster.name} for {damage} damage!")
            if monster_hp <= 0:
                break
            time.sleep(1)
            damage = max(monster_attack - player_defense, 1)
            player_hp -= damage
            stdscr.addstr(1, 0, f"{monster.name} attacks you for {damage} damage!")
        else:
            damage = max(monster_attack - player_defense, 1)
            player_hp -= damage
            stdscr.addstr(0, 0, f"{monster.name} attacks you for {damage} damage!")
            if player_hp <= 0:
                break
            time.sleep(1)
            damage = max(player_attack + weapon_damage - 1, 1)
            monster_hp -= damage
            stdscr.addstr(1, 0, f"You attack {monster.name} for {damage} damage!")

        stdscr.addstr(3, 0, f"{monster.name} HP: {monster_hp}")
        stdscr.addstr(4, 0, f"Your HP: {player_hp}")
        stdscr.refresh()
        time.sleep(1)
    ...
    while True:
        ...
        if stats["class_path"] == "Swordsman":
            player_skills.extend([
                Skill("Power Strike", "A strong physical attack.", "active", mana_cost=0, cooldown=10, required_class="Swordsman", rank="C", level_required=1),
                Skill("Quick Jab", "A fast, weak jab.", "active", mana_cost=2, cooldown=1, required_class="Swordsman", rank="D", level_required=1),
            ])
        elif stats["class_path"] == "Mage":
            player_skills.extend([
                Skill("Fireball", "A ball of fire.", "active", mana_cost=8, cooldown=7, required_class="Mage", rank="C", level_required=1),
                Skill("Arcane Bolt", "A bolt of arcane energy.", "active", mana_cost=4, cooldown=1, required_class="Mage", rank="D", level_required=1),
            ])
        
        elif stats["class_path"] == "Cleric":
            player_skills.extend([
                Skill("Heal", "A pulse of holy light heals allies.", "active", mana_cost=10, cooldown=3, required_class="Mage", rank="C", level_required=1),
                Skill("Arcane Bolt", "A bolt of arcane energy.", "active", mana_cost=4, cooldown=1, required_class="Mage", rank="D", level_required=1),
            ])
            ...
            if current_tile in zone["tiles"]:
                ...
                if zone_name == "slime":
                    if random.random() < 0.20:
                        if not battle(stdscr, "Slime", stats, inventory, level_range):
                            player_x, player_y = 0, 0
                elif zone_name == "goblin":
                    if random.random() < 0.10:
                        if not battle(stdscr, "Goblin", stats, inventory, level_range):
                            player_x, player_y = 0, 0
                elif zone_name == "orc":
                    if random.random() < 0.08:
                        if not battle(stdscr, "Orc", stats, inventory, level_range):
                            player_x, player_y = 0, 0
                elif zone_name == "mixed":
                    if random.random() < 0.15:
                        monsters = [("Slime", 0.4), ("Goblin", 0.3), ("Kobold", 0.2), ("Orc", 0.1)]
                        chosen = pick_monster(monsters)
                        if not battle(stdscr, chosen, stats, inventory, level_range):
                            player_x, player_y = 0, 0
